...
def placeNoteBlock(loc, configWay, hitNum, voiceMax, minDelay, repeaterNum, processedList, gameName):

    if configWay[0] == 1:
        for j in range(0, hitNum):
            for k in range(0, len(processedList[j][1])):      # the number of notes in noteProcessedList[j]
                gameName.setNoteBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), processedList[j][1][k])

        if configWay[1] == 1:      # harp/piano timbre
            for j in range(0, hitNum):
                for k in range(0, len(processedList[j][1])):
                    gameName.setBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y - 1, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), block.GRASS.id)
        elif configWay[1] == 2:      # double bass timbre
            for j in range(0, hitNum):
                for k in range(0, len(processedList[j][1])):
                    gameName.setBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y - 1, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), block.WOOD.id)
        elif configWay[1] == 3:      # glockenspiel timbre
            for j in range(0, hitNum):
                for k in range(0, len(processedList[j][1])):
                    gameName.setBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y - 1, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), block.GOLD_BLOCK.id)
        elif configWay[1] == 4:      # flute timbre
            for j in range(0, hitNum):
                for k in range(0, len(processedList[j][1])):
                    gameName.setBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y - 1, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), block.CLAY.id)
        elif configWay[1] == 5:      # chime timbre
            for j in range(0, hitNum):
                for k in range(0, len(processedList[j][1])):
                    gameName.setBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y - 1, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), block.PACKED_ICE.id)
        elif configWay[1] == 6:      # guitar timbre
            for j in range(0, hitNum):
                for k in range(0, len(processedList[j][1])):
                    gameName.setBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y - 1, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), block.WOOL.id)
        elif configWay[1] == 7:      # xylophone timbre
            for j in range(0, hitNum):
                for k in range(0, len(processedList[j][1])):
                    gameName.setBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y - 1, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), block.BONE_BLOCK.id)
        ...
        for j in range(0, hitNum):
            for k in range(0, len(processedList[j][1])):
                ...
                if 24 <= processedList[j][1][k] < 48:      # medium range, use harp/piano timbre
                    gameName.setNoteBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), processedList[j][1][k] - 24)
                    gameName.setBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y - 1, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), block.GRASS.id)
                if 48 <= processedList[j][1][k] <= 72:      # high range, use glockenspiel timbre
                    gameName.setNoteBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), processedList[j][1][k] - 48)
                    gameName.setBlock(loc.x + (- voiceMax + 1 + 2 * k), loc.y - 1, loc.z + 4 + processedList[j][0] / minDelay * (2 + repeaterNum), block.GOLD_BLOCK.id)
...
def placeRepeater(loc, hitNum, voiceMax, minDelay, repeaterNum, processedList, gameName):
    for i in range(- voiceMax + 1, voiceMax, 2):
        for j in range(0, int(processedList[-1][0] / minDelay)):
            if int(minDelay % 4) == 0:
                gameName.setBlock(loc.x + i, loc.y, loc.z + 6 + repeaterNum - 1 + j * (2 + repeaterNum), block.UNPOWERED_REPEATER.id, 2 + 12)      # delay = 4
            else:
                gameName.setBlock(loc.x + i, loc.y, loc.z + 6 + repeaterNum - 1 + j * (2 + repeaterNum), block.UNPOWERED_REPEATER.id, 2 + 4 * (int(minDelay % 4) - 1))      # delay != 4
            for k in range(0, repeaterNum - 1):      # range(0, 0) = []
                gameName.setBlock(loc.x + i, loc.y, loc.z + 6 + k + j * (2 + repeaterNum), block.UNPOWERED_REPEATER.id, 2 + 12)      # delay = 4
...
def placeRedstoneWire(loc, hitNum, voiceMax, minDelay, repeaterNum, processedList, gameName):
    
    for i in range(- voiceMax + 1, voiceMax, 2):
        for j in range(0, int(processedList[-1][0] / minDelay)):
            gameName.setBlock(loc.x + i, loc.y, loc.z + 5 + j * (2 + repeaterNum), block.REDSTONE_WIRE.id)
    ...
    for k in range(0, hitNum):
        for l in range(len(processedList[k][1]), voiceMax):
            gameName.setBlock(loc.x + (- voiceMax + 1 + 2 * l), loc.y, loc.z + 4 + processedList[k][0] / minDelay * (2 + repeaterNum), block.REDSTONE_WIRE.id)
    ...
    for m in range(0, int(processedList[-1][0] / minDelay)):
        if m * minDelay not in [processedListMember[0] for processedListMember in processedList]:
            for n in range(- voiceMax + 1, voiceMax, 2):
                gameName.setBlock(loc.x + n, loc.y, loc.z + 4 + m * (2 + repeaterNum), block.REDSTONE_WIRE.id)

# --------------------
# placeTorch
# --------------------

def placeTorch(loc, zBoundary, hitNum, voiceMax, repeaterNum, gameName):
    if hitNum == 1:
        ...
        for i in range(- voiceMax, voiceMax + 1, 2):
            ...
            while True:
                m += 3
                if 6 + m * (2 + repeaterNum) >= zBoundary[1]:
                    break
                gameName.setBlock(loc.x + i, loc.y, loc.z + 6 + m * (2 + repeaterNum), block.TORCH.id)

# --------------------
# placeBaseLine
# --------------------

def placeBaseLine(loc, voiceMax, gameName):

    # the base line can only spread redstone signal to 15 columns at most
    for i in range(- voiceMax + 1, voiceMax):
        gameName.setBlock(loc.x + i, loc.y, loc.z + 2, block.REDSTONE_WIRE.id)

    for j in range(- voiceMax + 1, voiceMax, 2):
        gameName.setBlock(loc.x + j, loc.y, loc.z + 3, block.UNPOWERED_REPEATER.id, 2)
