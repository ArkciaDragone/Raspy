...
public class RemoteSession {
	...
	protected ArrayDeque<EntityDeathEvent> deathEventQueue = new ArrayDeque<EntityDeathEvent>();

	protected ArrayDeque<PlayerLoginEvent> playerLoginEventQueue = new ArrayDeque<PlayerLoginEvent>();

	protected ArrayDeque<PlayerQuitEvent> playerQuitEventQueue = new ArrayDeque<PlayerQuitEvent>();

	protected ArrayDeque<PlayerRespawnEvent> playerRespawnEventQueue = new ArrayDeque<PlayerRespawnEvent>();
	...
	public void queueEntityDeathEvent(EntityDeathEvent event){
		deathEventQueue.add(event);
	}

	public void queuePlayerLoginEvent(PlayerLoginEvent event){
		playerLoginEventQueue.add(event);
	}

	public void queuePlayerQuitEvent(PlayerQuitEvent event){
		playerQuitEventQueue.add(event);
	}

	public void queuePlayerRespawnEvent(PlayerRespawnEvent event){
		playerRespawnEventQueue.add(event);
	}
	...
	protected void handleCommand(String c, String[] args) {
		...
		try {
			...
			// New Code Expanded by ClayLivince
			// world.setNoteBlock(posX, posY, posZ, pitch, blockdata)
			} else if (c.equals("world.setNoteBlock")) {
				Location loc = parseRelativeBlockLocation(args[0], args[1], args[2]);
				updateBlock(world, loc, 25, (args.length > 4 ? Byte.parseByte(args[4]) : (byte) 0));
				NoteBlock block = (NoteBlock) world.getBlockAt(loc).getState();
				if (Integer.parseInt(args[3]) > 24 || Integer.parseInt(args[3]) < 0) {
					plugin.getLogger().warning("Note pitch should be 0 <= pitch <= 24.");
				} else {
					block.setNote(new Note(Integer.parseInt(args[3])));
					boolean flag = block.update();
					if (!flag) plugin.getLogger().warning("Failed to update the block.");
				}

			// world.setNote(posX, posY, posZ, pitch)
			} else if (c.equals("world.setNote")) {
				Location loc = parseRelativeBlockLocation(args[0], args[1], args[2]);
				...
				if (block instanceof NoteBlock) {
					if (Integer.parseInt(args[3]) > 24 || Integer.parseInt(args[3]) < 0) {
						...
						((NoteBlock) block).setNote(new Note(Integer.parseInt(args[3])));
						block.update();
						boolean flag = block.update();
						if (!flag) plugin.getLogger().warning("Failed to update the block.");
					}
				} else {
					plugin.getLogger().warning("The block at the target position is not a NoteBlock.");
				}

			//server.executeCommand
			} else if (c.equals("server.executeCommand")) {
				String commandMessage = "";
				int count;
				for (count = 0; count < args.length; count++) {
					commandMessage = commandMessage + args[count] + " ";
				}
				commandMessage = commandMessage.substring(0, commandMessage.length() - 1);
				boolean flag = server.dispatchCommand(server.getConsoleSender(), commandMessage);
				if (!flag)
					plugin.getLogger().warning("There is a problem in " + commandMessage + ", please check your command.");

			//event.player.death
			} else if (c.equals("events.player.death")) {
				StringBuilder bdr = new StringBuilder();
				EntityDeathEvent event;
				while ((event = deathEventQueue.poll()) != null) {
					if (event instanceof PlayerDeathEvent) {
						...
						bdr.append(name);
						bdr.append(",");
						bdr.append(player.getEntityId());
						bdr.append(",");
						bdr.append(blockLocationToRelative(loc));
						if (deathEventQueue.size() > 0) {
							bdr.append("|");
						}
					}
				}
			...
			} else if (c.equals("events.player.login")) {
				StringBuilder bdr = new StringBuilder();
				PlayerEvent event;
				while ((event = playerLoginEventQueue.poll()) != null) {
						...
						bdr.append(",");
						bdr.append(blockLocationToRelative(loc));
						if (playerLoginEventQueue.size() > 0) {
							bdr.append("|");
						}
					}
				send(bdr.toString());

			} else if (c.equals("events.player.quit")) {
				StringBuilder bdr = new StringBuilder();
				PlayerEvent event;
				while ((event = playerQuitEventQueue.poll()) != null) {
						Player player = event.getPlayer();
						String name = player.getPlayerListName();
						Location loc = player.getLocation();
						bdr.append(name);
						...
						bdr.append(",");
						bdr.append(blockLocationToRelative(loc));
						if (playerQuitEventQueue.size() > 0) {
							bdr.append("|");
					}
				}
				send(bdr.toString());

			} else if (c.equals("events.player.respawn")){
				StringBuilder bdr = new StringBuilder();
				PlayerEvent event;
				while ((event = playerRespawnEventQueue.poll()) != null){
						Player player = event.getPlayer();
						String name = player.getPlayerListName();
						Location loc = player.getLocation();
						bdr.append(name);
						...
						bdr.append(",");
						bdr.append(blockLocationToRelative(loc));
						if (playerRespawnEventQueue.size() > 0) {
							bdr.append("|");
						}
				}
				send(bdr.toString());

			//entity.addPotionEffect(int entityid, string potionname, int duration, int amplifier, boolean ambient, boolean particles, int red, int green, int blue)
				...
				if(entity != null){
					if(entity instanceof LivingEntity){
						...
						boolean particle = Boolean.parseBoolean(args[5]);
						if(args.length > 6 && particle) {
							int red = Integer.parseInt(args[6]);
							int green = Integer.parseInt(args[7]);
							int blue = Integer.parseInt(args[8]);
							((LivingEntity) entity).addPotionEffect(new PotionEffect(potiontype, duration, amplifier, ambient, particle, Color.fromRGB(red, green, blue)));
						} else ((LivingEntity) entity).addPotionEffect(new PotionEffect(potiontype, duration, amplifier, ambient, particle));
					} else {
						plugin.getLogger().warning("The entity is not a LivingEntity.");
					}
				} else {
					plugin.getLogger().warning("Invalid entity.");
				}
